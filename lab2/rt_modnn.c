...
void init_all(void){
	...
	int index=0;
	int schedulable=1;
	Subtask currentList[MAX_SUBTASK_PER_TASK];
	...
	if(mode_input==RUN) {
		...
		for (i=0; i<num_subtask; i++){
			...
			if (subtask_ptrs[i]->core==-1) {
				printk(KERN_INFO "UNSCHEDULABLE CASE when add subtask_ptrs[%d], and drop the subtask", i);
				schedulable=0;
				subtask_ptrs[i]->parent->num-=1;
				currentList=*(subtask_ptrs[i]->parent->subtask_list);
				for(j=subtask_ptrs[i]->idx_in_task;j<(subtask_ptrs[i]->parent->num)-1;j++){
					subtask_ptrs[i]->parent->subtask_list[j]=subtask_ptrs[i]->parent->subtask_list[j + 1];
					subtask_ptrs[i]->parent->subtask_list[j + 1]->idx_in_task=j;
				}
				printk(KERN_INFO "now task[%i] has %d tasks",subtask_ptrs[i]->parent->index, subtask_ptrs[i]->parent->num);
				//subtask_ptrs[i]->core = 3;
				//cpu_load[3]+=subtask_ptrs[i]->utilization;
				//subtask_ptrs[i]->idx_in_core = cores[3].num;
				//cores[3].num = cores[3].num + 1;
		...
		if(schedulable==0){
				...
				for (i=0;i<num_task;i++){
					...
					for (j=0;j<tasks[i].num;j++){
						...
						printk(KERN_INFO "1. task%d, subtask %d==%d??, execution time is %d", i,j, tasks[i].subtask_list[j].idx_in_task, tasks[i].subtask_list[j].execution_time);
						total_exec_time = total_exec_time + tasks[i].subtask_list[j].execution_time;
				//		printk(KERN_INFO "2. //////%d %d %d %d %p", tasks[i].period, tasks[i].num, tasks[i].index, tasks[i].execution_time, tasks[i].subtask_list);
						tasks[i].subtask_list[j].cumul_exec_time = total_exec_time;
				//		printk(KERN_INFO "3. //////%d %d %d %d %p", tasks[i].period, tasks[i].num, tasks[i].index, tasks[i].execution_time, tasks[i].subtask_list);
		...
		//TODO: WRONG if some subtasks are dropped
  		if(sub->idx_in_task==0) { //the first one
  			...
  			if (ktime_compare(current_time, expect_next)<0) { //current_time<expect_next
  				...
  				wake_up_process(sub->sub_thread);//immediately wake up it
  			}
  		}
